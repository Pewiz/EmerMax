...
function manejarOdometro() {
    const anchoVentana = window.innerWidth;
    const odometroOriginal = document.querySelector('.elementoDerecha #odometer');
    const cajaDerechaMovil = document.querySelector('.cajaDerechaMovil p');

    // Si el ancho de la ventana es menor o igual a 650px, mover el odómetro
    if (anchoVentana <= 650) {
        if (odometroOriginal) {
            // Elimina el odómetro de su posición original
            odometroOriginal.remove();
        }

        // Inserta el odómetro en la cajaDerechaMovil si no está ya insertado
        if (!document.querySelector('.cajaDerechaMovil #odometer')) {
            // Crea el ícono '+' y el div del odómetro
            const iconoMas = document.createElement('i');
            iconoMas.textContent = '+'; // Crea el ícono de '+'
            
            const nuevoOdometro = document.createElement('div');
            nuevoOdometro.id = 'odometer';
            nuevoOdometro.classList.add('odometer');
            nuevoOdometro.textContent = '0'; // Valor inicial del odómetro

            // Inserta el ícono '+' y luego el odómetro en el contenedor móvil
            cajaDerechaMovil.insertBefore(nuevoOdometro, cajaDerechaMovil.firstChild);
            cajaDerechaMovil.insertBefore(iconoMas, nuevoOdometro);
        }
    } else {
        // Si el ancho de la ventana es mayor a 650px, asegúrate de que el odómetro esté en su lugar original
        const contenedorOriginal = document.querySelector('.elementoDerecha');
        if (!document.querySelector('.elementoDerecha #odometer')) {
            const nuevoOdometroOriginal = document.createElement('div');
            nuevoOdometroOriginal.id = 'odometer';
            nuevoOdometroOriginal.classList.add('odometer');
            nuevoOdometroOriginal.textContent = '0'; // Valor inicial del odómetro

            contenedorOriginal.insertBefore(nuevoOdometroOriginal, contenedorOriginal.firstChild);
        }

        // Elimina el odómetro de la cajaDerechaMovil si existe
        const odometroMovil = document.querySelector('.cajaDerechaMovil #odometer');
        if (odometroMovil) {
            odometroMovil.remove();
        }
    }
}

// Llama a la función cuando la ventana cambia de tamaño
window.addEventListener('resize', manejarOdometro);
